...
bool initDisplay(void) {
  ...
  if (i2c_detect(oled_addr)) {
    config.config |= CFG_LCD;
    Log.verbose(F(" SSD1306"));
  } else if (i2c_detect(++oled_addr)) {
    config.config |= CFG_LCD;
    Log.verbose(F(" SH1106"));
  } else {
...
void drawFrameTinfo(OLEDDisplay *oled, OLEDDisplayUiState *state, int16_t x, int16_t y) {
  ...
  if (!(status & STATUS_TINFO)) {
    ...
    oled->drawString(x + 64, 10, F("Teleinfo"));
...
void drawFrameRF(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
    ...
    if (!(status & STATUS_RFM)) {
      display->setTextAlignment(TEXT_ALIGN_CENTER);
      display->drawString(x + 64, 10, F("Radio"));
      display->drawString(x + 64, 24, F("not"));
      display->drawString(x + 64, 38, F("initialized"));
    }
    else if (!got_first) {
      display->setTextAlignment(TEXT_ALIGN_CENTER);
      display->drawString(x + 64, 14, F("No radio data"));
      display->drawString(x + 64, 34, F("received yet"));
    } else {
